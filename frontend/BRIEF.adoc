...
. Write the production `Dockerfile` (multi-stage) and verify `docker build -t resume-ranker-frontend:prod .` produces a working image. Run with `docker run -p 3000:3000 resume-ranker-frontend:prod` and verify the built site loads.
