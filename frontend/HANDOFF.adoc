...
| Container | `resumeranker-frontend`
...
docker exec resumeranker-frontend pnpm exec svelte-check --tsconfig ./tsconfig.json

# Unit tests
docker exec resumeranker-frontend pnpm exec vitest run

# Production deploy — push the image to a registry and pull it on the target
docker build -t resumeranker-frontend:latest .
docker tag resumeranker-frontend:latest <registry>/<you>/resumeranker-frontend:1.0
docker push <registry>/<you>/resumeranker-frontend:1.0
# Then on the target host:
docker run -d --name resumeranker-frontend --restart unless-stopped \
  -p 80:3000 -e VITE_API_BASE_URL=https://api.example.com \
  <registry>/<you>/resumeranker-frontend:1.0
----
