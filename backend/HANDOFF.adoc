...
| `Dockerfile.dev` runs `uv sync` at _build time_ as the brief specified — this works because the venv lives in a named volume (`resume-ranker-backend-venv`) so the bind mount doesn't clobber it. | No frontend-style "runtime install" needed; uv's behavior plays nicer with Docker than pnpm's did.
