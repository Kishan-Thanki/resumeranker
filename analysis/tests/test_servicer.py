...
def make_usage() -> dict[str, int | float | None]:
    return {
        "input_tokens": 500,
        "output_tokens": 150,
        "total_tokens": 650,
...
class TestAnalysisEngineServicer:
    ...
    async def test_successful_analysis_returns_complete_response(
        ...
        assert response.result_json == final_analysis.model_dump_json(
            by_alias=True,
            exclude_none=True,
        )
    ...
    async def test_llm_analysis_error_returns_expected_failure_response(
        ...
        assert response.success is False
        assert (
            response.error_message == "ERR_RATE_LIMIT_EXCEEDED: LLM rate limit reached."
        )
        assert response.result_json == ""
        assert response.model == ""
